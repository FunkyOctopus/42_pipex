...
/*   Updated: 2021/09/24 17:49:54 by olgerret         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	prnt->out = open(argv[prnt->argc_parent - 1], O_RDWR | O_CREAT | O_TRUNC, 0666);
	if (prnt->out == -1)
	{
...
int	parent(t_pipes *p, t_parent *prnt, char **argv, char **envp)
{
	...
	if (waitpid(-1, NULL, 0) && check_file(argv[prnt->argc_parent - 1], 'W') != -1)
	{
